...
Example Perm3_refl123 : Perm3 [1;2;3] [1;2;3].
Proof.
...
Proof.
  intros E5.
  inversion E5 as [| n E3 n3].
  inversion E3 as [| n' E1 n1].
  inversion E1.
Qed.
(** [] *)
...
Lemma ev_double' : forall n, ev (n + n).
Proof.
  induction n.
  - apply ev_0.
  - simpl.
...
Inductive total_relation : nat -> nat -> Prop :=
  | is_le : forall {n m} (lt : n <= m), total_relation n m
  | is_gt : forall {n m} (gt : m < n), total_relation n m
.
...
Lemma le_suc : forall n m, n <= m -> S n <= S m.
Proof.
  intros.
  induction H.
  - apply le_n.
...
Theorem le_S_inv : forall n m, S n <= S m -> n <= m.
Proof.
  intros n m H.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
     ...
     the rule c1, c2, c3 defines the relation [R a b c] iff [a + b = c]
     the rule c4 and c5 is admissible.
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | S_match : forall x l1 l2, subseq l1 l2 -> subseq (x :: l1) (x :: l2)
  | S_skip : forall x l1 l2, subseq l1 l2 -> subseq l1 (x :: l2)
.
...
Theorem subseq_split : forall x l1 l2, subseq (x :: l1) l2 ->
  exists hd tl, l2 = hd ++ (x :: tl) /\ subseq l1 tl.
Proof.
  intros x l1 l2.
  ...
  - inversion H; subst.
    + exists []. exists l2.
      ...
      * reflexivity.
      * assumption.
    + apply IHl2 in H2 as [hd' [tl' [Happ Hsub]]].
      exists (a :: hd'). exists tl'.
      split.
      * simpl.
        rewrite Happ.
        reflexivity.
...
Theorem subseq_tail : forall x l1 l2,
  subseq (x :: l1) l2 -> subseq l1 l2.
Proof.
  intros x l1.
  destruct l1; intros.
  - apply S_nil.
  - apply subseq_split in H.
    destruct H as [ht [tl [Happ Hsub]]].
    rewrite Happ.
    apply subseq_app'.
    apply S_skip.
    apply Hsub.
...
Proof.
  ...
  - inversion H; subst.
    + apply S_nil.
    + apply subseq_split in H0 as [hd [tl [Happ Hsub]]].
      subst.
      apply subseq_app'.
      apply S_match.
...
(* 
    ...
    - [R 2 [1;0]] via c2, c2, c1
    - [R 1 [1;2;1;0]] via c3c2, c3c3c2, c2, c2, c1
    - [R 6 [3;2;1;0]] Nope the first element will be geq 5
...
Definition concat {T}: list (list T) -> list T.
Proof.
  intros l.
...
Proof.
  ...
  }
  {
    ...
    - (* Char *) exists [t0]. apply MChar.
    - (* App *) simpl in H.
      assert (re_not_empty re1 = true
      /\ re_not_empty re2 = true) as [H1 H2].
      {
        destruct (re_not_empty re1) eqn:eq1;
        destruct (re_not_empty re2) eqn:eq2;
        auto.
      }
      apply IHre1 in H1 as [s1 HM1].
      apply IHre2 in H2 as [s2 HM2].
      exists (s1 ++ s2).
      apply (MApp s1).
      apply HM1.
      apply HM2.
    - (* Union *) simpl in H.
      ...
      + apply IHre1 in H1 as [s1 HM1].
        exists s1. apply MUnionL. apply HM1.
      + apply IHre2 in H2 as [s2 HM2].
        exists s2. apply MUnionR. apply HM2.
    - (* Star *) exists []. apply MStar0.
  }
Qed.
(** [] *)
...
Search (?a + ?b <= ?c + ?d).
Check add_le_cases: forall n m p q : nat,
  n + m <= p + q -> n <= p \/ m <= q.

Search (length (?a ++ ?b) = length ?a + length ?b).
Check app_length: forall (X : Type) (l1 l2 : list X),
  length (l1 ++ l2) = length l1 + length l2.

Lemma le_plus_le_l : forall n m p, n + m <= p -> n <= p.
Proof.
  intros n m.
  induction m; intros.
  - rewrite add_0_r in H.
...
Lemma le_plus_le_r : forall n m p, n + m <= p -> m <= p.
Proof.
  intros n.
  induction n; intros.
  - apply H.
...
Proof.
  ...
  - (* MApp *)
    ...
    apply add_le_cases in Hlen.
    destruct Hlen as [Hlen1 | Hlen2].
    + (* pump in left  part *)
      clear IH2.
      apply IH1 in Hlen1 as [s1' [s2' [s3' [Happ Hpump]]]].
      ...
      * split.
        -- apply Hpump.
        -- destruct Hpump as [_ Hpump].
           intros m.
           specialize (Hpump m).
           assert (s1' ++ napp m s2' ++ s3' ++ s2 = (s1' ++ napp m s2' ++ s3') ++ s2) as H.
           {
             rewrite <- app_assoc.
             rewrite <- app_assoc.
             reflexivity.
    ...
    + (* pump in right part *)
      clear IH1.
      apply IH2 in Hlen2 as [s1' [s2' [s3' [Happ Hpump]]]].
      ...
      * split.
        ...
        -- destruct Hpump as [_ Hpump].
           intros m.
           specialize (Hpump m).
           assert ((s1 ++ s1') ++ napp m s2' ++ s3' = s1 ++ (s1' ++ napp m s2' ++ s3')) as H.
           {
             rewrite <- app_assoc.
             reflexivity.
           }
  ...
  - (* MUnionL *)
    intros H.
    simpl in H.
    apply le_plus_le_l in H.
    specialize (IH H) as [s1' [s2' [s3' H']]].
    destruct H' as [Happ [Hnemp Hpump]].
...
Proof.
  ...
  - intros [].
  - intros H'.
    specialize (eqbP  n m) as Heq.
    destruct (eqb n m) eqn:eqn.
    + inversion Heq.
      rewrite H in Hcount.
      simpl in Hcount.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | empty : nostutter []
  | single : forall x, nostutter [x]
  | cons_xy : forall l x y, x <> y -> nostutter (y :: l) -> nostutter (x :: y :: l)
.
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
  | merge_nil : merge [] [] []
  | merge_lhs : forall x xs ys l, merge xs ys l -> merge (x :: xs) ys (x :: l)
  | merge_rhs : forall y xs ys l, merge xs ys l -> merge xs (y :: ys) (y :: l)
.

Lemma merge_l_nil {X : Type} : forall (l1 l2 : list X), merge [] l1 l2 -> l1 = l2.
Proof.
  intros l1.
...
Lemma merge_r_nil {X : Type} : forall (l1 l2 : list X), merge l1 [] l2 -> l1 = l2.
Proof.
  intros l1.
  induction l1; intros; inversion H; subst.
  - reflexivity.
...
Proof.
  ...
  induction H as [| x xs ys l H IH | y xs ys l H IH]; intros Hxs Hys.
  - reflexivity.
  - simpl in Hxs.
    destruct Hxs as [Hx Hxs].
    simpl.
    rewrite Hx.
    specialize (IH Hxs Hys).
    f_equal.
    apply IH.
  - simpl in Hys.
    destruct Hys as [Hy Hys].
    simpl.
    rewrite Hy.
    specialize (IH Hxs Hys).
    apply IH.
...
Inductive pal {X:Type} : list X -> Prop :=
  ...
  | pal_match : forall x xs (H : pal xs), pal (x :: xs ++ [x])
.
...
Definition list_tail {X:Type} (l:list X) :
    l = [] \/ exists x l', l = l' ++ [x].
Proof.
  induction l as [ | y l].
  - left. reflexivity.
  - right.
    destruct IHl as [H | [x [l' H]]].
    + exists y. exists [].
      rewrite H.
      reflexivity.
    + rewrite H.
...
Lemma tail_nil {X : Type} : forall x (l : list X), ~ (l ++ [x] = []).
Proof.
  intros.
  destruct l ; discriminate.
Qed.

Lemma hd_eq {X : Type} : forall x x' (l l' : list X),
  x :: l = x' :: l' -> x = x' /\ l = l'.
Proof.
  intros.
  inversion H.
...
Lemma rev_inj {X} : forall (l l' : list X), rev l = rev l' -> l = l'.
Proof.
  intros.
...
Lemma tail_eq {X : Type} : forall x x' (l l' : list X),
  l ++ [x] = l' ++ [x'] -> x = x' /\ l = l'.
Proof.
  intros x x' l l'.
  rewrite <- (rev_involutive l).
  rewrite <- (rev_involutive l').
  change (rev (rev l) ++ [x]) with (rev (x :: rev l)).
  change (rev (rev l') ++ [x']) with (rev (x' :: rev l')).

  intros H.
  assert (x :: rev l = x' :: rev l').
  {
    apply rev_inj.
    apply H.
  }
...
(**
   x :: l = rev l ++ [x]
   x :: l ++ [x] = rev (l ++ [x]) ++ [x]
   x :: l = rev (l ++ [x]) = rev [x] ++ rev l = x :: rev l
   l = rev l
 ...
 *)
Fixpoint palindrome_converse' {X: Type} n :
  forall (l: list X) (Hguard : length l <= n),
  l = rev l -> pal l.
Proof.
  destruct n; intros.
  - destruct l.
  ...
  - destruct l.
    + apply pal_nil.
    + simpl in H.
      specialize (list_tail l) as Htl.
      destruct Htl as [Hnil | [y [l' Hcons]]].
      * rewrite Hnil.
        apply pal_one.
      * subst.
        change (x :: l' ++ [y]) with ((x :: l') ++ [y]) in H.
        apply tail_eq in H as [Hxy Htl].
        subst.
        rewrite rev_app_distr in Htl.
        simpl in Htl.
        apply hd_eq in Htl as [_ Hl].
        apply pal_match.

        simpl in Hguard.
        rewrite app_length in Hguard.
        rewrite add_comm in Hguard.
        simpl in Hguard.
        apply le_S_inv in Hguard.
        destruct n as [| n'].
        -- inversion Hguard.
        -- apply le_S_inv in Hguard.
           apply palindrome_converse' with (n := n').
           ++ exact Hguard.
           ++ exact Hl.
Qed.
...
Proof.
  intros X l H.
  apply palindrome_converse' with (n := length l).
  - apply le_n.
  - exact H.
Qed.
(** [] *)
...
Inductive NoDup {X : Type} : list X -> Prop
  :=
  | nodup_nil : NoDup []
  | nodup_cons : forall x xs, ~ In x xs -> NoDup xs -> NoDup (x :: xs)
  .

Lemma in_app {X : Type} : forall x (l1 l2 : list X),
...
Theorem nodup_app_disjoint_nodups {X : Type} : forall (l1 l2 : list X)
  (H1 : NoDup l1)
  (H2 : NoDup l2)
...
Proof.
  ...
  - simpl.
    apply nodup_cons.
    + intros Hin.
      apply in_app in Hin as [Hin1 | Hin2].
      * inversion H1; subst.
        contradiction.
...
Proof.
  ...
  - intros [].
  - intros.
    destruct H as [Hhere | Hlater].
    + subst.
      exists []. exists l.
      reflexivity.
    + specialize (IHl Hlater) as [l1 [l2 H]].
      subst.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | rep_here : forall x l, In x l -> repeats (x :: l)
  | rep_there : forall x l, repeats l -> repeats (x :: l)
.
...
Proof.
  ...
  {
    destruct H as [[H0 H1] | H].
    - change (a :: s) with ([] ++ (a :: s)).
      apply MApp; assumption.
    - destruct H as [s0 [s1 [Hs [H0 H1]]]].
      subst.
      change (a :: s0 ++ s1) with ((a :: s0) ++ s1).
      apply MApp; assumption.
  }
Qed.
(** [] *)
...
Lemma match_union_nil {T : Type} : forall (re1 re2 : reg_exp T),
  ([ ] =~ Union re1 re2)
  <->
  ([ ] =~ re1) \/ ([ ] =~ re2).
Proof.
  ...
  { (* <- *)
    intros [H | H];
    try (apply MUnionL; apply H);
    try (apply MUnionR; apply H).
  }
Qed.

Lemma match_app_nil {T : Type} : forall (re1 re2 : reg_exp T),
  ([ ] =~ App re1 re2)
  <->
  ([ ] =~ re1) /\ ([ ] =~ re2).
Proof.
  ...
  { (* -> *)
    ...
    (* 3/4 of the cases are contradiction *)
    try inversion H1.
    simpl.
    split; assumption.
  }
  { (* <- *)
    intros [H1 H2].
    apply MApp with (s1 := []) (s2 := []); assumption.
  }
Qed.
...
Proof.
  ...
  intros re.
  induction re.
  - (* empty *) apply ReflectF.
    unfold not.
    intros H.
    inversion H.
  - (* epsilon *) apply ReflectT.
    apply MEmpty.
  - (* char *) apply ReflectF.
    unfold not.
    intros H.
  ...
  - (* app *)
    simpl;
    inversion IHre1 as [H1 B1 | H1 B1]; subst; clear IHre1;
    inversion IHre2 as [H2 B2 | H2 B2]; subst; clear IHre2;
    simpl;
    constructor;
    (* most of the cases are contradiction *)
    try (intros H; apply match_app_nil in H as [HM1 HM2]; contradiction).
    + apply MApp with (s1 := []) (s2 := []); assumption.
  - (* union *)
    simpl;
    inversion IHre1 as [H1 B1 | H1 B1]; subst; clear IHre1;
    ...
    simpl;
    constructor;
    try ((apply MUnionL; assumption) || (apply MUnionR; assumption)).
    + intros H. apply match_union_nil in H as [H | H]; contradiction.
  - (* Kleen star *)
    constructor.
    apply MStar0.
Qed.
...
(** [d a re] the derivative of [re] with respect to char [a] 
*)
...
Fixpoint derive (a : ascii) (re : reg_exp ascii) : reg_exp ascii
  := match re with
     ...
     | App r1 r2 =>
         let r := App (derive a r1) r2 in
         if match_eps r1
         then Union r (derive a r2)
         else r
...
Fixpoint splits {A : Type} (l : list A) : list (list A * list A) :=
  match l with
  | [] => [([], [])]
  | x :: l =>
      ([], x :: l) ::
      map (fun '(h, t) => (x :: h, t)) (splits l)
  end.
